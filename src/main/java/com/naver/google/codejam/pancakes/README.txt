...
해결 방법
결국 전부 +혹은 -로 맞춘뒤 해결을 해야한다고 생각함.
그리고 앞쪽에서부터 끊어서 뒤집을 수 있으므로 뒷쪽부터 맞춰야한다고생각.(사실 앞에서부터하던 뒤에서부터하던 똑같음)
따라서 맨 뒷쪽에있는 팬케이크조각에 맞춰서 계속 뒤집는다.
그리고 마지막 팬케이크가 +일 경우 최종적으로 뒤집을 필요가없으므로 끝.
-일 경우엔 최종적으로 전부 뒤집는게 추가되므로 +1을 해주면된다.