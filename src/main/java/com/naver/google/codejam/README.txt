...
주소 - https://code.google.com/codejam/contest/1403486/dashboard
...
해결방법
줄이 변하는 때 : 주가 바뀔 때, 달이 바뀔 때

총 줄의 개수는 줄이 변할때 +1이다.
그리고 줄이 변하는 개수는 아래와 같다.
주가 바뀔때 + 달이 바뀔때 - 둘이 같이 바뀔 때

따라서 몇주인지 구하고, 몇달인지 구한뒤 같이 바뀔때를 구해주면 된다.
같이 바뀔때는 최소공배수를 구해서 구하면 된다.